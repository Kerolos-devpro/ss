...
namespace Test.Controllers
{
    public class CartController : Controller
    {
        ...
        public IActionResult AddToCart(int Count, int ProductId)
        {
            ...
            var existingCartItem = cartRepository.GetAll(expression:e => e.UserId == userId && e.ProductId == ProductId).FirstOrDefault();
            ...
            if (existingCartItem != null)
            {
                ...
                TempData["SuccessMessage"] = "Product count has been updated in your cart.";
            }
            else
            {
        ...
        public async Task<IActionResult> CheckoutSuccess(string couponCode)
        {
            ...
            string userMessage = $"Hello , Mr :{UserFullName}, thank you for making the order " +
                $", the order will be received during week from now , " +
            ...
            foreach (var item in addedToCartItems)
            {
                ...
                if (product != null)
                {
                    ...
                    if (product.Stock == 3)
                    {
                        var message = $"these products : $/$ {product.Name} $/$ will become out of stock, the available quantity is {product.Stock} , please ship order";
                        await emailSender.SendEmailAsync(SD.AdminEmail, "Out Of Stock Products", message);
                    }
                    if (product.Stock == 0)
                    {
                        var message = $"this product : $/$ {product.Name} $/$ is already out of stock , please ship order";
                        await emailSender.SendEmailAsync(SD.AdminEmail, "Out Of Stock Products", message);
                    }
        ...
        public async Task<IActionResult> CashSuccess(string couponCode)
        {
            ...
            foreach (var item in addedToCartItems)
            {
                ...
                if (product != null)
                {
                    ...
                    if (product.Stock == 0)
                    {
                        var message = $"these products : $/$ {product.Name} $/$ is already out of stock";
                        await emailSender.SendEmailAsync(SD.AdminEmail, "Out Of Stock Products", message);
                    }
                    productRepository.Update(product);
                }
